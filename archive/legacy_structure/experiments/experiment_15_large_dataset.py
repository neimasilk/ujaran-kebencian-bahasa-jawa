"""
EXPERIMENT 15: Large Balanced Dataset (39K samples)
===================================================
Using the full balanced_dataset.csv with best practices
"""
import os
import sys
...
class Config:
    ...
    data_path: str = "data/standardized/balanced_dataset.csv"
    output_dir: str = "results/experiment_15_large_dataset"
    model_output_dir: str = "models/experiment_15_large_dataset"
    max_length: int = 128
    batch_size: int = 32
...
def main():
    ...
    print(f"[EXP15] Data: {len(df)} samples", flush=True)

    # Show label distribution
    label_counts = df['label'].value_counts().sort_index()
    print(f"[EXP15] Label dist: {dict(label_counts)}", flush=True)
    ...
    print(f"[EXP15] Train={len(train_df)}, Val={len(val_df)}, Test={len(test_df)}", flush=True)

    # Compute class weights
    class_weights = compute_class_weights(df['label'].values)
    print(f"[EXP15] Class weights: {class_weights.tolist()}", flush=True)

    # Load model
    tokenizer = AutoTokenizer.from_pretrained(cfg.model_path)
    model = AutoModelForSequenceClassification.from_pretrained(cfg.model_path, num_labels=4)
    print(f"[EXP15] Model: IndoBERT Base", flush=True)
    ...
    # Train
    print("[EXP15] Training started...", flush=True)
    train_result = trainer.train()
    print(f"[EXP15] Training completed in {train_result.metrics['train_runtime']:.0f}s", flush=True)

    # Evaluate
    print("[EXP15] Evaluating...", flush=True)
    ...
    # Save results
    results = {
        'experiment': 'Experiment 15 - Large Balanced Dataset',
