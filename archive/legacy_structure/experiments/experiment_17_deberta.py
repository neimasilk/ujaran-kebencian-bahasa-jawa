"""
EXPERIMENT 17: DeBERTa V3 - SOTA Architecture
==========================================
DeBERTa V3 is SOTA on many NLU tasks. Disentangled attention
should capture Javanese nuance better than BERT.
"""
import os
import sys
...
class Config:
    ...
    output_dir: str = "results/experiment_17_deberta"
    model_output_dir: str = "models/experiment_17_deberta"
    max_length: int = 128
    batch_size: int = 16
...
def main():
    ...
    print(f"[EXP17] Model: DeBERTa V3 Base", flush=True)
    ...
    print("\n" + "="*50, flush=True)
    print("EXPERIMENT 17 RESULTS (DeBERTa V3)", flush=True)
    print("="*50, flush=True)
    print(f"Accuracy:  {accuracy:.4f}", flush=True)
    print(f"F1-Macro:  {f1_macro:.4f}", flush=True)
    print(f"Baseline:  0.8138 (IndoBERT)", flush=True)
    ...
    # Save results
    results = {
        'experiment': 'Experiment 17 - DeBERTa V3',
