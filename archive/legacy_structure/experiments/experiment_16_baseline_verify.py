"""
EXPERIMENT 16: Verify Baseline with Phase 3+4 Dataset
======================================================
Reproduce the 81.38% result with exact same configuration
"""
import os
import sys
...
class Config:
    ...
    output_dir: str = "results/experiment_16_baseline_verify"
    model_output_dir: str = "models/experiment_16_baseline_verify"
    max_length: int = 128
    batch_size: int = 16
...
def main():
    ...
    print(f"[EXP16] Data: {len(df)} samples", flush=True)

    # Show label distribution
    label_counts = df['label'].value_counts().sort_index()
    print(f"[EXP16] Label dist: {dict(label_counts)}", flush=True)

    # Split - exactly 80/10/10
    ...
    print(f"[EXP16] Train={len(train_df)}, Val={len(val_df)}, Test={len(test_df)}", flush=True)

    # NO class weights - using label smoothing instead
    class_weights = None

    # Load model
    tokenizer = AutoTokenizer.from_pretrained(cfg.model_path)
    model = AutoModelForSequenceClassification.from_pretrained(cfg.model_path, num_labels=4)
    print(f"[EXP16] Model: IndoBERT Base", flush=True)
    ...
    # Training args - exact same as Exp 6A
    ...
    # Train
    print("[EXP16] Training started...", flush=True)
    train_result = trainer.train()
    print(f"[EXP16] Training completed in {train_result.metrics['train_runtime']:.0f}s", flush=True)

    # Evaluate
    print("[EXP16] Evaluating...", flush=True)
    ...
    print("\n" + "="*50, flush=True)
    print("EXPERIMENT 16 RESULTS", flush=True)
    print("="*50, flush=True)
    print(f"Accuracy:  {accuracy:.4f}", flush=True)
    print(f"F1-Macro:  {f1_macro:.4f}", flush=True)
    print(f"Expected:  0.8138 (Exp 6A)", flush=True)
    print(f"Match:     {abs(f1_macro - 0.8138) < 0.01}", flush=True)
    print("="*50, flush=True)
    ...
    # Save results
    results = {
        'experiment': 'Experiment 16 - Verify Baseline',
        'model': 'indobert-base-p1',
        'dataset': 'phase3_phase4_combined',
